function moreDetails(siteObj) {
    ...
    db.collection("users").doc(siteChose).collection('Ticket').where('Timestamp', '>=', new Date().setHours(0, 0, 0, 0)).where('Timestamp', '<=', new Date().setHours(23, 59, 59, 0)).get().then(snapshot => {
        ...
        if (doc.data().siteName.toLowerCase() != "admin")
            sites.innerHTML += ` <div class="col-7 bg-warning p-3 m-2  text-center  ">
             <h4 class="text-uppercase">
                ...
                <button class="btn  border-danger bg-dark text-white mt-4 " onclick="moreDetails('${doc.data().uid}')">View
                    Details</button>
            </div>`
    })
})
...
filter.addEventListener('submit', (e) => {
    ...
    document.getElementById('site-details').innerHTML = '';
    var fileTitle;
    db.collection('UserProfile').where('uid', '==', siteChose).get().then(snapshot => {
            fileTitle = Date.now() +  snapshot.docs[0].data().siteName;
    })
    db.collection("users").doc(siteChose).collection('Ticket').where('Timestamp', '>=', startDate).where('Timestamp', '<=', endDate).get().then(snapshot => {
        var Details = [];
